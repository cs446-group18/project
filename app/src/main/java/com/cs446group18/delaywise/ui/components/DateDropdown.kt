...
@Composable
fun DateDropdown(suggestions: List<Instant>, defaultDate: Instant, changeDateFunc: suspend (LocalDate) -> Unit, isReadOnly: Boolean, timeZone: TimeZone) {
        ...
        DropdownMenu(
            ...
            suggestions.forEach { label ->
                DropdownMenuItem(onClick = {
                    mutableState = label.formatAsDate(timeZone)
                    changeDateFunc
                    expanded = false
                }, text = { Text(label.formatAsDate(timeZone)) })
            }
        }
    }
}