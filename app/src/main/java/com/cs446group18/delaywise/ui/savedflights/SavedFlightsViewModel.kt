...
class SavedFlightsViewModel : ViewModel() {
    private val _savedFlightAirportState =
        MutableStateFlow<UiState<Pair<List<SavedFlightEntity>, List<SavedAirportEntity>>>>(UiState.Loading())
    val savedFlightAirportState: StateFlow<UiState<Pair<List<SavedFlightEntity>, List<SavedAirportEntity>>>> =
        _savedFlightAirportState
